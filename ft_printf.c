...
/*   Updated: 2022/11/09 15:17:47 by yes-slim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_format(const char c, va_list ap, int *len)
{
	if (c == 'c')
		...
		ft_print_nbr_uns(va_arg(ap, unsigned int), len);
	else if (c == 'x')
		ft_print_hex_low(va_arg(ap, unsigned int), len);
	else if (c == 'X')
		ft_print_hex_up(va_arg(ap, unsigned int), len);
	else if (c == 'p')
	{
		(*len) += write(1, "0x", 2);
